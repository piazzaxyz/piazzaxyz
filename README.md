<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=200&section=header&text=Eduardo%20Piazza&fontSize=60&fontColor=ffffff&fontAlignY=38&desc=Full-Stack%20Developer%20%7C%20Computer%20Engineering%20%40%20UERGS&descAlignY=58&descSize=16&animation=fadeIn" width="100%"/>

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=A78BFA&center=true&vCenter=true&width=600&lines=Desenvolvedor+Full-Stack+Jr.+%F0%9F%9A%80;TypeScript+%7C+React+%7C+Python+%7C+FastAPI;Porto+Alegre%2C+RS+%F0%9F%87%A7%F0%9F%87%B7;19+anos+%7C+Agromai+%7C+UERGS;+900%2B+commits+no+GitHub+de+trabalho)](https://git.io/typing-svg)

</div>

---

<div align="center">

[![Portfolio](https://img.shields.io/badge/Portfólio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://pzdeveloper-sage.vercel.app/#home)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/eduardo-siqueira-de-melo-piazza/)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/piazzadev/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:piazza.vsc@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/piazzaxyz)

</div>

---

## `> whoami`

```ts
const eduardo = {
  nome:        "Eduardo Piazza",
  idade:       19,
  localização: "Porto Alegre, RS — Brasil 🇧🇷",
  empresa:     "Agromai 🌱  (Desenvolvedor Full-Stack Jr.)",
  educação:    "Engenharia da Computação @ UERGS",
  foco:        ["Frontend moderno", "Backend escalável", "IA & LLMs"],
  vibeCode:    ["Claude Code", "Cursor"],
  status:      "🚀 Aberto a oportunidades",
};
```

---

## `> stack --frontend`

<div align="center">

[![My Skills](https://skillicons.dev/icons?i=ts,js,react,nextjs,vite,html,css,tailwind&perline=8&theme=dark)](https://skillicons.dev)

| Tecnologia | Nível |
|---|---|
| TypeScript / JavaScript | ████████████░ Avançado |
| React + Next.js | ████████████░ Avançado |
| Tailwind CSS + GSAP | ██████████░░ Sólido |
| Vite | ██████████░░ Sólido |
| HTML / CSS | ████████████ Expert |

</div>

---

## `> stack --backend`

<div align="center">

[![My Skills](https://skillicons.dev/icons?i=python,FastAPI,nodejs,fastapi,celery,docker,postgres,git&perline=8&theme=dark)](https://skillicons.dev)

| Tecnologia | Nível |
|---|---|
| Python + FastAPI | ████████████░ Avançado |
| Celery (async tasks) | █████████░░░ Sólido |
| Node.js / FastAPI | ████████░░░░ Sólido |
| Docker | ████████░░░░ Sólido |
| PostgreSQL | ███████░░░░░ Intermediário |

</div>

---

## `> stack --extras`

<div align="center">

[![My Skills](https://skillicons.dev/icons?i=gcp,vscode,cursor,github,figma&perline=8&theme=dark)](https://skillicons.dev)

</div>

<div align="center">

![Discord](https://img.shields.io/badge/Discord%20Bots-5865F2?style=for-the-badge&logo=discord&logoColor=white)
![WhatsApp](https://img.shields.io/badge/WhatsApp%20Chatbot-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)
![LLM](https://img.shields.io/badge/LLM%20Training-FF6B6B?style=for-the-badge&logo=openai&logoColor=white)
![PayPal](https://img.shields.io/badge/PayPal%20Gateway-00457C?style=for-the-badge&logo=paypal&logoColor=white)
![MetaAPI](https://img.shields.io/badge/MetaAPI-FF8C00?style=for-the-badge&logo=meta&logoColor=white)

> Experiência com **bots de Discord**, **chatbots de WhatsApp**, **MetaAPI**, e gateways de pagamento como **pagar.me**, **abacatepay**, **PayPal** e **CartPanda**.
> Treinamento e validação de **LLMs** — definição de intents, pipelines de testagem e avaliação de modelos.

</div>

---

## `> stats --github`

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=piazzaxyz&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true&bg_color=0d1117&title_color=a78bfa&icon_color=a78bfa&text_color=c9d1d9&custom_title=GitHub%20Stats"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=piazzaxyz&theme=tokyonight&layout=compact&langs_count=8&hide_border=true&bg_color=0d1117&title_color=a78bfa&text_color=c9d1d9&custom_title=Linguagens%20Favoritas"/>

</div>

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=piazzaxyz&theme=tokyonight&hide_border=true&background=0d1117&stroke=a78bfa&ring=a78bfa&fire=ff6b6b&currStreakLabel=a78bfa&sideLabels=a78bfa&dates=c9d1d9" width="500"/>

</div>

<div align="center">

> 📌 **+900 commits** registrados no GitHub corporativo (Agromai) — não refletidos nas estatísticas públicas acima.

</div>

---

## `> projects --pinned`

<div align="center">

### 🔐 Horus — TUI de QA & Cybersegurança

[![Horus](https://github-readme-stats.vercel.app/api/pin/?username=piazzaxyz&repo=horus&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=a78bfa&icon_color=a78bfa&text_color=c9d1d9)](https://github.com/piazzaxyz/horus)

> Terminal User Interface focada em **Quality Assurance** e **Cybersegurança**.
> Construída com Python, pensada para automação de testes e análise de segurança diretamente no terminal.

---

### 🌐 Portfólio — pzdeveloper

[![Portfolio Preview](https://img.shields.io/badge/🌐_Ver_Portfólio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://pzdeveloper-sage.vercel.app/#home)

> Portfólio atual em produção. Em breve receberá uma **reformulação completa** com nova estética, em colaboração com **designers externos**. Fique de olho. 👀

</div>

---

## `> experience --timeline`

```
2024 – atual  │  Desenvolvedor Full-Stack Jr. @ Agromai
              │  React · TypeScript · Python · FastAPI · Celery · Docker
              │  +900 commits no repositório corporativo
              │
2024 – atual  │  Engenharia da Computação @ UERGS
              │  Porto Alegre, RS
              │
Contínuo      │  Projetos freelance & open source
              │  Bots · Chatbots · E-commerce · Gateways de Pagamento · LLMs
```

---

## `> contact`

<div align="center">

Sou colaborativo, adaptável e estou sempre em busca de evoluir.
Se você tem um projeto interessante ou uma oportunidade, vamos conversar!

[![LinkedIn](https://img.shields.io/badge/Me%20adiciona%20no%20LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/eduardo-siqueira-de-melo-piazza/)
[![Instagram](https://img.shields.io/badge/Me%20segue%20no%20Insta-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/piazzadev/)
[![Email](https://img.shields.io/badge/Manda%20um%20Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:piazza.vsc@gmail.com)

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=120&section=footer&animation=fadeIn" width="100%"/>

<sub>Made with 💜 by Eduardo Piazza · Powered by <a href="https://github.com/piazzaxyz">@piazzaxyz</a></sub>

</div>
